...
public class AttributesComponentPanel extends VerticalLayout
{
	...
	public AttributesComponentPanel(MessageSource msg, AttributesGrid main)
	{
		...
		bus.addListener(event -> setInput(event.entity() == null ? null : event.entity(), event.group()),
	...
	private void setInput(EntityWithLabel owner, Group group)
	{
		...
		if (owner == null)
		{
			...
			add(new HorizontalLayout(VaadinIcon.EXCLAMATION_CIRCLE_O.create(), new Span(msg.getMessage("Attribute.noEntitySelected"))));
			return;
		}
		add(new Html("<h5>" + msg.getMessage("Attribute.caption", owner, group.getDisplayedNameShort(msg).getValue(msg)) + "</h5>"));
