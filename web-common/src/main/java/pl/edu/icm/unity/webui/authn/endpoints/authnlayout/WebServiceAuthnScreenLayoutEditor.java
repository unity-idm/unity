...
import pl.edu.icm.unity.webui.authn.endpoints.authnlayout.AuthnLayoutPropertiesHelper.AuthenticationLayoutContent;
...
public class WebServiceAuthnScreenLayoutEditor extends CustomComponent
{
			...
			Supplier<List<String>> authnOptionSupplier)
	{
		...
		initUI();
		Consumer<LayoutColumn> removeListener = c -> removeColumn(c);
		...
		if (columns.isEmpty())
		{
			columns.add(new LayoutColumn(msg, removeListener, removeElementListener, dragStart, dragStop));
		}
	...
	private void initUI()
	{
		...
		addColumnButton.addClickListener(ev -> {
			columns.add(new LayoutColumn(msg, e -> removeColumn(e), e -> removeElementFromColumns(e),
	...
	private void removeColumn(LayoutColumn c)
	{
