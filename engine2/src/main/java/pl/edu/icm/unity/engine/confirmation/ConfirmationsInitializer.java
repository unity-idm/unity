...
public class ConfirmationsInitializer implements ServerInitializer
{
	...
	private AttributeTypeSupport attributesMan;
	private IdentityTypeSupport identitiesMan;
	...
	public ConfirmationsInitializer(
			...
			AttributeTypeSupport attributesMan,
			IdentityTypeSupport identitiesMan)
	{
	...
	public void run()
	{
		try
		{
			...
			{
				if (attributesMan.getSyntax(at).isVerifiable())
				{
			...
			{
				if (identitiesMan.getTypeDefinition(idType.getIdentityTypeProvider()).isVerifiable())
				{
