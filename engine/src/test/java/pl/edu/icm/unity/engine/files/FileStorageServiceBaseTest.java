...
public class FileStorageServiceBaseTest
{
	...
	public void testSaveAndRead() throws EngineException
	{
		...
		FileData fileData = storageService.readURI(uri, null);
	...
	public void shouldReadBase64ImageDataUri() throws IllegalURIException, EngineException
	{
		...
		FileData fdata = storageService.readURI(URIHelper.parseURI(uri), null);
		assertThat(fdata.getContents(), is(notNullValue()));
	}

	@Test
	public void shouldReadImageUrl() throws IllegalURIException, EngineException
	{
		String uri = "https://unity-idm.eu/site-wp/wp-content/uploads/2016/11/lezka.png";
		FileData fdata = storageService.readImageURI(URIHelper.parseURI(uri), "demo");
		assertThat(fdata.getContents(), is(notNullValue()));
	}
	...
	{
		storageService.stoarageFileInWorkspace(new String("test").getBytes(), "demo/demo/test.txt");	
		FileData fileFromWorkspace = storageService.readFileFromWorkspace("demo/demo/test.txt");	
	...
	{			
		Throwable exception = catchThrowable(() -> storageService.stoarageFileInWorkspace(new String("test").getBytes(), "../test2.txt"));
		ExceptionsUtils.assertExceptionType(exception, EngineException.class);
	}
}
