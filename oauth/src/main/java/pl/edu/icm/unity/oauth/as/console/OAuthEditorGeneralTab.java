...
class OAuthEditorGeneralTab extends CustomComponent implements EditorTab
{
	...
	private void refreshScope(boolean add, OIDCScopeValue value)
	{
		...
		if (scope.isPresent())
		{
			...
			newScope.setAttributes(scope.get().getAttributes());
			newScope.setDescription(scope.get().getDescription());
			newScope.setName(scope.get().getName());
	...
	private CollapsibleLayout buildScopesSection()
	{
		...
		scopesGrid.addCheckboxColumn(s -> s.isEnabled(), msg.getMessage("OAuthEditorGeneralTab.scopeEnabled"), 10).setResizable(true);
		scopesGrid.addTextColumn(s -> s.getDescription(), msg.getMessage("OAuthEditorGeneralTab.scopeDescription"), 10).setResizable(true);
		scopesGrid.addTextColumn(s -> s.getAttributes() != null ? String.join(",", s.getAttributes()) : "",
				msg.getMessage("OAuthEditorGeneralTab.scopeAttributes"), 10).setResizable(true);
