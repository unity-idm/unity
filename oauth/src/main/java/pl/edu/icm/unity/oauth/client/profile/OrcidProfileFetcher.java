...
{
	private static final Logger log = Log.getLogger(Log.U_SERVER_OAUTH, OrcidProfileFetcher.class);
	...
	{
		AccessToken clientAccessToken = getClientAccessToken(fetcherConfig);
		JSONObject userBio = fetchUserBio(userInfoEndpoint, fetcherConfig, attributesSoFar, clientAccessToken);
		return ProfileFetcherUtils.fetchFromJsonObject(userBio);
	}

	private AccessToken getClientAccessToken(ProfileFetcherConfig fetcherConfig) throws Exception
	{
		AuthorizationGrant clientGrant = new ClientCredentialsGrant();
		ClientID clientID = new ClientID(fetcherConfig.clientId());
		Secret clientSecret = new Secret(fetcherConfig.clientSecret());
		ClientAuthentication clientAuth = new ClientSecretBasic(clientID, clientSecret);
		Scope scope = new Scope("/read-public");

		URI tokenEndpoint = new URI(fetcherConfig.accessTokenEndpoint());
	...
	private JSONObject fetchUserBio(String bioEndpointBase, ProfileFetcherConfig fetcherConfig,
			Map<String, List<String>> attributesSoFar, AccessToken clientAccessToken) throws Exception
	{
		...
		new HttpRequestConfigurer().secureRequest(httpReq, fetcherConfig.validator(),
				fetcherConfig.hostNameCheckingMode());
		...
		if (resp.getEntityContentType() == null
				|| !ContentType.APPLICATION_JSON.matches(resp.getEntityContentType()))
			throw new AuthenticationException("Authentication was successful "
					+ "but there was a problem fetching user's profile information. "
