...
public class PlainProfileFetcher implements UserProfileFetcher
{
	...
	public AttributeFetchResult fetchProfile(BearerAccessToken accessToken,
			String userInfoEndpoint, ProfileFetcherConfig fetcherConfig,
			Map<String, List<String>> attributesSoFar) throws Exception
	{
		...
		HTTPRequest httpReq = new HTTPRequest(
				fetcherConfig.clientHttpMethodForProfileAccess(),
				uri.build().toURL());

		new HttpRequestConfigurer().secureRequest(httpReq, fetcherConfig.validator(),
				fetcherConfig.hostNameCheckingMode());

		if (fetcherConfig.clientAuthnModeForProfileAccess() == ClientAuthnMode.secretPost)
