...
public class LdapAttributeUtils
{
			...
			throws LdapException, EngineException
	{
		...
		case MEMBER_OF_AT:
			Map<String, GroupMembership> grps = identitiesMan
					.getGroups(new EntityParam(userEntity.getId()));
			if (grps != null)
			{
	 ...
	 */
	private Attribute addDynamicMappedAttribute(String name, Collection<AttributeExt> attrs,
			Entity userEntity) throws LdapException
	{
		Attribute da = null;

		for (String key : keys)
		{
			String atName = props.getValue(
					key + LdapServerProperties.ATTRIBUTES_MAP_LDAP_AT);
			String oidName = props.getValue(
					key + LdapServerProperties.ATTRIBUTES_MAP_LDAP_OID);
			String unityAttribute = props.getValue(
					key + LdapServerProperties.ATTRIBUTES_MAP_UNITY_ATRIBUTE);
			String unityIdentity = props.getValue(
					key + LdapServerProperties.ATTRIBUTES_MAP_UNITY_IDENTITY);

			if (name.equalsIgnoreCase(atName))
			{
				if (!unityAttribute.isEmpty())
				{
					da = mapUnityAttributeToLdapAttribute(attrs, unityAttribute,
							atName, oidName);
				} else if (!unityIdentity.isEmpty())
				{
					da = mapUnityIdentityToLdapAttribute(userEntity,
							unityIdentity, atName, oidName);
				} else
				{
					// error
				}
	 ...
	 * unity attribute specified by attributeName, or null if the unity
	 * attribute is not found in the collection.
	 * 
	 * @param unityAttrs
	 *                Collection of unity attributes
	 * @param attributeName
	 *                Name of the unity attribute
	 * @param uid
	 *                LDAP attribute name
	 * @param upId
	 *                LDAP attribute name (OID)
	 * @return LDAP attribute
	 * @throws LdapException
	 */
	private Attribute mapUnityAttributeToLdapAttribute(Collection<AttributeExt> unityAttrs,
			String attributeName, String uid, String upId) throws LdapException
	{
		...
		if (value != null)
		{
			...
			log.debug(String.format(
					"Mapped unity attribute [%s] to ldap attribute [%s/%s] with value: %s.",
					attributeName, uid, upId, value));
			return da;
		} else
		{
			log.warn(String.format(
					"Failed to map unity attribute [%s] to ldap attribute [%s/%s]. Cause: unity attribute not found.",
					attributeName, uid, upId));
			String availableAttributes = "";
			for (AttributeExt ae : unityAttrs)
			{
				availableAttributes += ae.getName() + ",";
			}
			log.debug(String.format("Available unity attributes [%s].",
					availableAttributes.substring(0,
	 ...
	 * @throws LdapException
	 */
	private Attribute mapUnityIdentityToLdapAttribute(Entity userEntity, String identityTypeId,
			String uid, String upId) throws LdapException
	{
		...
		if (id != null)
		{
			...
			da.add(id.getValue());
			log.debug(String.format(
					"Mapped unity identity of type [%s] to ldap attribute [%s/%s] with value: %s.",
					identityTypeId, uid, upId, id.getValue()));
			return da;
		} else
		{
			log.warn(String.format(
					"Failed to map unity identity to ldap attribute [%s/%s]. Cause: no unity identity of type %s found.",
					uid, upId, identityTypeId));
			...
			{
				log.debug(String.format("Available unity identities [%s].",
						availableIdentities.substring(0,
