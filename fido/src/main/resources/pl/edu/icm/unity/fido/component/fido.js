 ...
 */
window.pl_edu_icm_unity_fido_component_FidoComponent =
	function() {
		var self = this;
		...
		this.createCredentials = function(key, options) {
			try {
				...
				self.showInternalError("Fido registration failed", err.message);
			}
		};
		...
		this.getCredentials = function(key, options) {
			try {
				...
				self.showInternalError("Fido authentication failed", err.message);
			}
		};
	};
