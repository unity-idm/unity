...
/**
 * Note: default settings contian one system user with "cn=system administrator".
 */
public class LdapServerFacade
{
    ...
    public LdapServerFacade(String host, int port, BaseInterceptor interceptor, String nameOrNull, String workdir) {
        this.host_ = host;
        this.port_ = port;
    ...
    /**
     * Apache DS has to be initialised with schemas etc.
     * - this directory will be used to hold all the required configuration which is then loaded
     */
    private File prepareWorkDir(boolean delete_work_dir) throws IOException {
        ...
        if (shouldExtract) {
            SchemaLdifExtractor extractor = new DefaultSchemaLdifExtractor(
                ds_.getInstanceLayout().getPartitionsDirectory()
            );
    ...
    private void loadSettings() throws Exception {
        File schemaPartitionDirectory = new File(
            ds_.getInstanceLayout().getPartitionsDirectory(), "schema"
        );
        ...
        LdifPartition schemaLdifPartition = new LdifPartition( schemaManager, ds_.getDnFactory() );
        schemaLdifPartition.setPartitionPath( schemaPartitionDirectory.toURI() );
        SchemaPartition schemaPartition = new SchemaPartition( schemaManager );
        schemaPartition.setWrappedPartition( schemaLdifPartition );
        ds_.setSchemaPartition(schemaPartition);

        JdbmPartition systemPartition = new JdbmPartition(ds_.getSchemaManager(), ds_.getDnFactory());
        systemPartition.setId( "system" );
        systemPartition.setPartitionPath( new File( ds_.getInstanceLayout().getPartitionsDirectory(), systemPartition.getId() ).toURI() );
    ...
    public void addUser(String cn, String password, String displayName) throws LdapException {
        ...
        ds_.getPartitionNexus().add(new AddOperationContext(
                null, userDN, serverEntry)
        );
    }
