...
public class IdpStatisticTest extends AbstractBasicDAOTest<IdpStatistic>
{
	...
	protected IdpStatistic getObject(String id)
	{
		return IdpStatistic.builder().idpEndpointId("eid").idpEndpointName("ename").clientId("cid")
				.clientName("cname").status(Status.SUCCESSFUL).timestamp(Date.valueOf(LocalDate.now()))
				.build();
	}
	...
	{
		return IdpStatistic.builder().idpEndpointId("eid").idpEndpointName("ename").clientId("cid")
				.clientName("cname").status(Status.FAILED).timestamp(Date.valueOf(LocalDate.now())).build();
	}
	...
	public void shouldRemoveStatisticsOldStatistics()
	{
		tx.runInTransaction(() ->
		{
			...
			IdpStatistic toRemove = IdpStatistic.builder().idpEndpointId("eid2").idpEndpointName("ename2")
					...
					.timestamp(Date.valueOf(LocalDate.now().minusDays(2))).build();
			dao.create(s1);
			dao.create(s2);
			dao.create(toRemove);

			dao.deleteOlderThan(Date.valueOf(LocalDate.now().minusDays(1)));
	...
	public void shouldGetStatisticsFromPeriod()
	{
		tx.runInTransaction(() ->
		{
			...
			List<IdpStatistic> all = dao.getIdpStatistics(Date.valueOf(LocalDate.now().minusDays(1)),
					Date.valueOf(LocalDate.now()), 100);
			assertThat(all.size(), is(2));
			assertThat(all.contains(s1), is(true));
			assertThat(all.contains(s2), is(true));
		});
