...
class RequestProcessingPanel extends CustomComponent
{
	...
	void setRequest(RegistrationRequestState input)
	{
		...
		{
			NotificationPopup.showError(msg, msg.getMessage("RequestProcessingPanel.errorGetRegistrationForms"), e);
			return;
		}

		RegistrationForm form = findForm(forms, input.getRequest().getFormId(), input.getRequestId());
		setValueGeneric(input, form);
		requestType.setValue(msg.getMessage("RegistrationRequest.type.registration"));
	...
	void setRequest(EnquiryResponseState input)
	{
		...
		{
			NotificationPopup.showError(msg, msg.getMessage("RequestProcessingPanel.errorGetEnquiryForms"), e);
			return;
		}
		EnquiryForm form = findForm(forms, input.getRequest().getFormId(), input.getRequestId());
		
		setValueGeneric(input, form);
		requestType.setValue(msg.getMessage("RegistrationRequest.type.enquiry"));
		commentPanel.setInput(input);
